...
// Ensure the normalized product payload class is loaded.
require_once dirname(__DIR__) . '/distributors/class-fflhub-distributor-product-payload.php';

/**
 * Admin page for searching distributor products by UPC.
 */
class FFLHub_Admin_Page_Distributor_Products
{
    ...
    {
        add_submenu_page(
            FFLHub_Admin_Page::get_page_slug(),          // parent slug (FFL Hub settings page)
    ...
    public static function render_page(): void
    {
        ...
        $upc_value            = '';
        $global_error         = '';
        $selected_product     = null; // FFLHub_Distributor_Product_Payload|null
        $selected_dist_id     = '';
        $selected_dist_label  = '';
        $carrier_distributors = array(); // [dist_id => ['label' => ..., 'payload' => FFLHub_Distributor_Product_Payload, 'true_cost' => float|null, 'quantity' => int|null]]
        ...
        if ($action) {
            ...
            if ('' === $upc) {
                ...
                foreach ($distributors as $id => $distributor) {
                    ...
                    try {
                        ...
                        if ($product instanceof FFLHub_Distributor_Product_Payload) {
                            ...
                            $carrier_distributors[(string) $id] = array(
                                'label'     => $label,
                                'payload'   => $product,
                                'true_cost' => $true_cost,
                                'quantity'  => $quantity,
                ...
                if ('create' === $action && $selected_product instanceof FFLHub_Distributor_Product_Payload && empty($global_error)) {
        ...
        <div class="wrap">
            ...
            <?php if ($selected_product instanceof FFLHub_Distributor_Product_Payload) : ?>
                <?php
     ...
     *
     * @param string                             $upc
     * @param FFLHub_Distributor_Product_Payload $selected_product
    ...
    private static function create_woo_product_from_payload(
        string $upc,
        FFLHub_Distributor_Product_Payload $selected_product,
        ...
        $base_price     = (null !== $true_cost) ? $true_cost * (1 + $markup_percent) : $dealer_price;
        ...
        if (null === $recommended || $recommended <= 0) {
            return new \WP_Error(
                'fflhub_no_price',
        ...
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_LAST_TRUE_COST_META, $true_cost);
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_LAST_DEALER_PRICE_META, $dealer_price);
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_LAST_MAP_META, $max_map);
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_LAST_MSRP_META, $max_msrp);
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_LAST_COMPUTED_PRICE_META, $recommended);
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_MARKUP_MODE_META, 1);
        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_MARKUP_PERCENT_META, 0);

        $product->update_meta_data(FFLHub_Product_Meta::FFLHUB_FFL_REQUIRED_META, $ffl_required ? 1 : 0);
