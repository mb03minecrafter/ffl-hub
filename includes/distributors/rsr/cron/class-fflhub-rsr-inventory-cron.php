 ...
 */
class FFLHub_RSR_Inventory_Cron {
    ...
    public static function cron_update_inventory_from_qty_file(): void {
        ...
        // Need the distributor class for FTP creds.
        if ( ! class_exists( 'FFLHub_Distributor_RSR' ) ) {
            error_log( '[FFLHub][RSR Inventory Cron] ERROR: FFLHub_Distributor_RSR class not found.' );
            return;
        }
        ...
        $ok = FFLHub_RSR_FTP_Client::download_file(
            $remote_path,
            $local_path,
            $host,
            $username,
    ...
    protected static function apply_inventory_updates_from_file( string $file_path ): int {
        ...
        if ( ! class_exists( 'FFLHub_RSR_Fulfillment_Table' ) ) {
            error_log( '[FFLHub][RSR Inventory Cron] FFLHub_RSR_Fulfillment_Table class not found.' );
            return 0;
        }

        $live_table = FFLHub_RSR_Fulfillment_Table::get_live_table_name();
