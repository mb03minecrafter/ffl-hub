 ...
 */
class FFLHub_Lipseys_Pricing_Quantity_Cron
{
    ...
    public static function cron_refresh_pricing_quantity(): void
    {
        ...
        if (! class_exists('FFLHub_Lipseys_Fulfillment_Schema')) {
            error_log("[FFLHub][Lipsey's Price/Qty Cron] ERROR: FFLHub_Lipseys_Fulfillment_Schema class not found.");
            $log_timing('Early exit (missing schema)', $t_start);
            error_log("[FFLHub][Lipsey's Price/Qty Cron] ---- RUN END (ERROR) ----");
            return;
        }

        if (! class_exists('FFLHub_Lipseys_Fulfillment_Table')) {
            error_log("[FFLHub][Lipsey's Price/Qty Cron] ERROR: FFLHub_Lipseys_Fulfillment_Table class not found.");
            $log_timing('Early exit (missing table helper)', $t_start);
            error_log("[FFLHub][Lipsey's Price/Qty Cron] ---- RUN END (ERROR) ----");
            return;
        }
        $log_timing('Schema/table helper check', $t_step);

        // LipseysClient exists?
        $t_step = microtime(true);
        if (! class_exists('\lipseys\ApiIntegration\LipseysClient')) {
            error_log("[FFLHub][Lipsey's Price/Qty Cron] ERROR: LipseysClient class not found (autoload issue).");
