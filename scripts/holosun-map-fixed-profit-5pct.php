...
if (!in_array($mode, ['dry-run', 'commit'], true)) {
    fwrite(STDERR, "Usage: wp eval-file scripts/holosun-map-fixed-profit-5pct.php -- [dry-run|commit] [optional_limit]\n");
    exit(1);
}
 ...
 */
function fflhub_holosun_price_plan(float $cost, float $shipping, float $fee_fraction, float $margin_fraction): array
{
    ...
    $computed_price = (float)$best['computed_price'];
    $fixed_profit = (float)$best['fixed_profit'];
...
$report_path = '/tmp/fflhub_holosun_map_fixed_profit_5pct_' . gmdate('Y-m-d_His') . '.csv';
...
echo "==== Holosun MAP Fixed Profit 5% Script ====\n";
echo "Mode: {$mode}\n";
