...
/**
 * Read-only admin page for Lipsey\'s credit usage on processing orders.
 */
final class LipseysCreditLimitPage
{
    private const PAGE_SLUG = 'fflhub-lipseys-credit-limit';
    private const LIPSEYS_DIST_ID = 'lipseys';
    ...
    public function register_menu_page(): void
    {
        add_submenu_page(
            ...
            __('Lipsey\'s Credit Limit', 'ffl-hub'),
            __('Lipsey\'s Credit Limit', 'ffl-hub'),
            'manage_options',
            self::PAGE_SLUG,
            [$this, 'render_page']
...
?>
        <div class="wrap fflhub-lipseys-credit-status">
            <?php $this->render_styles(); ?>
            <h1><?php esc_html_e('Lipsey\'s Credit Limit', 'ffl-hub'); ?></h1>
            <p>
                <?php esc_html_e('This page shows Lipsey\'s job-line entries for WooCommerce orders currently in Processing status.', 'ffl-hub'); ?>
            </p>
            <p>
                <?php esc_html_e('Completed orders are excluded here.', 'ffl-hub'); ?>
            </p>
            <p class="fflhub-lipseys-callout">
                <?php esc_html_e('Lipsey\'s ordering is automatic. Manual ordering is not required on this page, and this page does not write to job rows.', 'ffl-hub'); ?>
            </p>
...
<?php
    ...
    private function build_credit_status_data(array $jobs): array
    {
        ...
        * }> $entries */
        ...
        $credit_limit = $this->get_lipseys_credit_limit();
    ...
    private function get_lipseys_credit_limit(): float
    {
        $limit = Options::get_distributor_credit_limit(self::LIPSEYS_DIST_ID, self::DEFAULT_CREDIT_LIMIT);

        /** @var float|int|string $filtered */
        $filtered = apply_filters('fflhub_lipseys_credit_limit', $limit);
    ...
    private function render_summary_cards(array $data): void
    {
        ...
        ?>
        <div class="fflhub-lipseys-summary-grid">
            <section class="fflhub-lipseys-card">
                <h2><?php esc_html_e('Processing Orders', 'ffl-hub'); ?></h2>
                <div class="fflhub-lipseys-metric"><?php echo esc_html((string) ((int) $data['processing_order_count'])); ?></div>
                <p><?php esc_html_e('WooCommerce orders in Processing status.', 'ffl-hub'); ?></p>
            </section>

            <section class="fflhub-lipseys-card">
                <h2><?php esc_html_e('Lipsey\'s Jobs', 'ffl-hub'); ?></h2>
                <div class="fflhub-lipseys-metric"><?php echo esc_html((string) ((int) $data['job_count'])); ?></div>
                <p><?php esc_html_e('Job rows attached to those processing orders.', 'ffl-hub'); ?></p>
            </section>

            <section class="fflhub-lipseys-card is-ok">
                <h2><?php esc_html_e('Running Totals', 'ffl-hub'); ?></h2>
                <div class="fflhub-lipseys-metric"><?php echo esc_html((string) ((int) $data['total_quantity'])); ?></div>
                <p>
                    <?php
                    echo esc_html(
                        sprintf(
                ...
                <div class="fflhub-lipseys-note">
                    <?php esc_html_e('Running totals exclude rows already marked success.', 'ffl-hub'); ?>
                </div>
            </section>

            <section class="fflhub-lipseys-card <?php echo esc_attr($credit_card_class); ?>">
                <h2><?php esc_html_e('Credit Limit Usage', 'ffl-hub'); ?></h2>
                <div class="fflhub-lipseys-metric">
                    <?php
                    echo esc_html(
                        sprintf(
                            '%s / %s',
    ...
    private function render_running_totals_table(array $data): void
    {
        ...
        if (empty($rows)) {
            ?>
            <p><?php esc_html_e('No Lipsey\'s line entries found for processing orders.', 'ffl-hub'); ?></p>
            <?php
            return;
        }
        ?>
    ...
    private function render_styles(): void
    {
        ...
        <style>
            ...
            .fflhub-lipseys-card.is-ok {
                border-color: #a7f3d0;
                background: #ecfdf5;
            }
            .fflhub-lipseys-card.is-danger {
                border-color: #fecaca;
                background: #fef2f2;
            }
            .fflhub-lipseys-card h2 {
                margin: 0 0 8px;
                font-size: 15px;
            }
            .fflhub-lipseys-metric {
                font-size: 24px;
                font-weight: 700;
                line-height: 1.2;
            }
            .fflhub-lipseys-card p {
                margin: 8px 0 0;
                color: #50575e;
            }
            .fflhub-lipseys-note {
            ...
            .fflhub-lipseys-credit-status table code {
