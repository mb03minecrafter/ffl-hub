...
<p style="margin:0 0 14px;"><?php echo sprintf(esc_html__('Your quoted price for %1$s is %2$s plus shipping, which includes %3$s in savings.', 'ffl-hub'), esc_html($context->product_name), esc_html($context->final_price_display), esc_html($context->coupon_amount_display)); ?></p>
<p style="margin:0 0 14px;"><?php echo sprintf(esc_html__('When you are ready, enter code %1$s at checkout. It is worth %2$s and is valid through %3$s (48 hours).', 'ffl-hub'), esc_html($context->coupon_code), esc_html($context->coupon_amount_display), esc_html($context->expires_display)); ?></p>
