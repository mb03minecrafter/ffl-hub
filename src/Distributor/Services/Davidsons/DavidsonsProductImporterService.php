...
/**
 * Imports Davidson's full catalog CSV into the staging table.
 *
 * Primary path:
 * - LOAD DATA LOCAL INFILE for speed.
 *
 * Fallback path:
 * - PHP CSV parsing + batched INSERT IGNORE.
 */
class DavidsonsProductImporterService
{
    ...
    /**
     * Import from uploads/fflhub-davidsons/davidsons_inventory.csv
     */
    public function import_from_downloaded_file(): int
    {
        $uploads   = wp_upload_dir();
        $base_dir  = trailingslashit((string) ($uploads['basedir'] ?? '')) . 'fflhub-davidsons';
        $file_path = trailingslashit($base_dir) . 'davidsons_inventory.csv';

        if (!file_exists($file_path) || !is_readable($file_path)) {
            $this->log_debug('[FFLHub][Davidsons Import] File missing or unreadable at ' . $file_path);
            return 0;
        }
    ...
    public function import_fulfillment_file(string $file_path): int
    {
        if ($this->can_use_load_data_local_infile()) {
            ...
            $this->log_debug('[FFLHub][Davidsons Import] LOAD DATA path failed, falling back to PHP importer.');
        }
    ...
    private function can_use_load_data_local_infile(): bool
    {
        ...
        $this->log_debug(
            sprintf(
                '[FFLHub][Davidsons Import][DEBUG] LOAD DATA check: mysql_ok=%s, php_ok=%s, result=%s',
                $mysql_ok ? 'true' : 'false',
                $php_ok ? 'true' : 'false',
                $result ? 'true' : 'false'
            )
    ...
    private function import_fulfillment_file_via_load_data(string $file_path): int
    {
        ...
        if (!file_exists($file_path) || !is_readable($file_path)) {
            $this->log_debug('[FFLHub][Davidsons Import][LOAD DATA] file missing or not readable at ' . $file_path);
            return -1;
        }
        ...
        $sql = "
            ...
            SET
                ...
                inventory_quantity = CAST(IFNULL(NULLIF(TRIM(BOTH '\\r' FROM TRIM(@c7)), '') + 0, 0) AS CHAR),
                upc                = TRIM(BOTH '#' FROM TRIM(BOTH '\\r' FROM TRIM(@c8))),

                manufacturer      = TRIM(BOTH '\\r' FROM TRIM(@c9)),
                item_type         = TRIM(BOTH '\\r' FROM TRIM(@c10)),
                model             = TRIM(BOTH '\\r' FROM TRIM(@c11)),
                caliber_gauge     = TRIM(BOTH '\\r' FROM TRIM(@c12)),
                action            = TRIM(BOTH '\\r' FROM TRIM(@c13)),
                capacity          = TRIM(BOTH '\\r' FROM TRIM(@c14)),
                finish            = TRIM(BOTH '\\r' FROM TRIM(@c15)),
                stock_frame_grips = TRIM(BOTH '\\r' FROM TRIM(@c16)),
                sights            = TRIM(BOTH '\\r' FROM TRIM(@c17)),
                barrel_length     = TRIM(BOTH '\\r' FROM TRIM(@c18)),
                overall_length    = TRIM(BOTH '\\r' FROM TRIM(@c19)),
                features          = TRIM(BOTH '\\r' FROM TRIM(@c20)),

                allocation_status = CASE
                                        WHEN IFNULL(NULLIF(TRIM(BOTH '\\r' FROM TRIM(@c7)), '') + 0, 0) > 0
                                        THEN 'in_stock'
                                        ELSE 'out_of_stock'
                                    END,
                ffl_required      = CASE
                                        WHEN UPPER(TRIM(BOTH '\\r' FROM TRIM(@c10))) REGEXP 'PISTOL|REVOLVER|RIFLE|SHOTGUN|HANDGUN|FIREARM|RECEIVER'
                                        THEN '1'
                                        ELSE '0'
                                    END,
                sot_required      = CASE
                                        WHEN UPPER(TRIM(BOTH '\\r' FROM TRIM(@c10))) REGEXP 'SUPPRESSOR|SILENCER|NFA'
                                        THEN '1'
                                        ELSE '0'
                                    END,
                dropship_enabled      = '0',
        ...
        try {
            ...
            if ($result === false) {
                $this->log_debug('[FFLHub][Davidsons Import][LOAD DATA] query failed: ' . (string) $wpdb->last_error);
                return -1;
            }

            // Enforce required identifiers for reliable downstream joins and product creation.
            $wpdb->query(
                "
                ...
                WHERE
                    ...
                    OR davidsons_item_number IS NULL
                    OR TRIM(davidsons_item_number) = ''
                "
            ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            $sig_approved_forced = SigDropshipApproval::apply_to_table('davidsons', $table_name);
            if ($sig_approved_forced > 0) {
                $this->log_debug(
                    sprintf('[FFLHub][Davidsons Import][LOAD DATA] sig_approved_forced=%d', $sig_approved_forced)
                );
            }
        } catch (\Throwable $e) {
            $this->log_debug('[FFLHub][Davidsons Import][LOAD DATA] exception: ' . $e->getMessage());
            return -1;
        }
        ...
        $this->log_debug(
            sprintf(
                '[FFLHub][Davidsons Import] import_fulfillment_file_via_load_data(): total=%.2f ms (sql=%.2f ms), rows=%d, ignore_lines=%d',
                $t_total_ms,
                $t_sql_ms,
                $rows,
                $ignore_lines
    ...
    private function import_fulfillment_file_via_php(string $file_path): int
    {
        ...
        if (!$handle) {
            $this->log_debug('[FFLHub][Davidsons Import] could not fopen ' . $file_path);
            return 0;
        }
        ...
        if (!is_array($header) || empty($header)) {
            fclose($handle);
            $this->log_debug('[FFLHub][Davidsons Import] missing/invalid header in ' . $file_path);
            return 0;
        }

        $parser = new DavidsonsProductParser();
        $header_map = $parser->build_header_map($header);

        if (!$parser->has_required_columns($header_map)) {
            fclose($handle);
            $this->log_debug('[FFLHub][Davidsons Import] header missing required columns item # / upc code');
            return 0;
        }
        ...
        $flush_batch = function () use (
            ...
            if ($result !== false) {
                ...
                $batch_failures++;
                $this->log_debug('[FFLHub][Davidsons Import] Batch INSERT failed: ' . (string) $wpdb->last_error);
            }
        ...
        // Match LOAD DATA cleanup behavior.
        $wpdb->query(
            "
            DELETE FROM {$table_name}
            WHERE
        ...
        $this->log_debug(
            sprintf(
                '[FFLHub][Davidsons Import] import_fulfillment_file_via_php(): total=%.2f ms, parse+loop=%.2f ms, db_flush=%.2f ms, rows=%d, skipped_missing_upc=%d, batch_flushes=%d, batch_failures=%d',
                $t_import_total_ms,
                $t_parse_ms,
                $t_flush_ms,
                $rows,
