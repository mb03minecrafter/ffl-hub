...
/**
 * ZandersCaRelayBatchCronService
 *
 * Runs every minute and processes Zanders CA relay rows in batch_pending status.
 ...
 */
final class ZandersCaRelayBatchCronService extends AbstractCronService
{
    ...
    private const LOCK_OPTION = 'fflhub_zanders_ca_relay_batch_lock';
    private const LOCK_TTL_SECONDS = 300;

    /** Runtime option names for batch behavior. */
    private const OPT_ENABLED            = 'fflhub_zanders_ca_relay_batch_enabled';
    private const OPT_DISPATCH_TIME      = 'fflhub_zanders_ca_relay_batch_dispatch_time';
    private const OPT_LOW_STOCK_THRESHOLD = 'fflhub_zanders_ca_relay_batch_low_stock_threshold';
    private const OPT_RETRY_DELAY_SECONDS = 'fflhub_zanders_ca_relay_batch_retry_delay_seconds';
    private const OPT_MAX_ROWS_PER_RUN    = 'fflhub_zanders_ca_relay_batch_max_rows_per_run';
    private const OPT_FORCE_FLUSH         = 'fflhub_zanders_ca_relay_batch_force_flush';
    private const OPT_LAST_SCHEDULED_FLUSH_AT_UTC = 'fflhub_zanders_ca_relay_batch_last_scheduled_flush_at_utc';
    ...
    ): void
    {
        ...
        $request = new DistributorOrderRequest(
            ...
            '',
            'Zanders CA relay batch aggregate [' . $batch_kind . ']',
            'direct_ship_non_ffl'
        );
    ...
    {
        // Format: ZANR-{firstWooOrderNumber}-{lastWooOrderNumber}
        // Keep each order token short so the final PO stays <= 22 chars (Zanders cap).
        if (empty($batch_candidates)) {
            return 'ZANR-' . gmdate('mdHi') . '-' . (string) wp_rand(1000, 9999);
        }
    ...
    private function sanitize_po_segment(string $raw): string
    {
        ...
        // 4 + 1 + 8 + 1 + 8 = 22 max total for "ZANR-{first}-{last}".
        if (strlen($raw) > 8) {
            $raw = substr($raw, -8);
        }
