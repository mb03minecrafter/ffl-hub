...
/**
 * LipseysCaRelayBatchCronService
 *
 * Runs every minute and processes Lipsey's CA relay rows in batch_pending status.
 * These are non-FFL direct-ship rows whose customer state is CA; they are sent
 * through the drop-ship lane, but shipped to the configured relay ship-to address.
 *
 * Behavior:
 ...
 */
final class LipseysCaRelayBatchCronService extends AbstractCronService
{
    ...
    private const LOCK_OPTION = 'fflhub_lipseys_ca_relay_batch_lock';
    private const LOCK_TTL_SECONDS = 300;

    /** Runtime option names for batch behavior. */
    private const OPT_ENABLED            = 'fflhub_lipseys_ca_relay_batch_enabled';
    private const OPT_DISPATCH_TIME      = 'fflhub_lipseys_ca_relay_batch_dispatch_time';
    private const OPT_LOW_STOCK_THRESHOLD = 'fflhub_lipseys_ca_relay_batch_low_stock_threshold';
    private const OPT_RETRY_DELAY_SECONDS = 'fflhub_lipseys_ca_relay_batch_retry_delay_seconds';
    private const OPT_MAX_ROWS_PER_RUN    = 'fflhub_lipseys_ca_relay_batch_max_rows_per_run';
    private const OPT_FORCE_FLUSH         = 'fflhub_lipseys_ca_relay_batch_force_flush';
    private const OPT_LAST_SCHEDULED_FLUSH_AT_UTC = 'fflhub_lipseys_ca_relay_batch_last_scheduled_flush_at_utc';
    ...
    public function run(): void
    {
        ...
        try {
            ...
            $jobs = OrderPlacementJobsRepository::find_jobs_for_ca_relay_batch_processing(
                $this->jobs_table,
                'lipseys',
                $now_mysql_utc,
                $max_rows
    ...
    ): void
    {
        ...
        // CA relay orders ship to the dealer/relay address, not the CA customer.
        $ship_to = DealerShipToResolver::resolve_relay();
        if (!($ship_to instanceof DistributorShipTo)) {
            ...
            foreach ($batch_candidates as $entry) {
                ...
                $job = $entry['job'];
                $this->mark_failed((int) $job->order_id, (string) $job->job_key_norm(), 'CA relay batch failed: missing relay ship-to address.');
            }
            return;
        }
        ...
        $request = new DistributorOrderRequest(
            ...
            '',
            'Lipseys CA relay batch aggregate [' . $batch_kind . ']',
            'direct_ship_non_ffl'
        );
    ...
    {
        // Format: LIPR-{firstWooOrderNumber}-{lastWooOrderNumber}
        // Keep each order token short so the final PO stays <= 22 chars (Lipseys cap).
        if (empty($batch_candidates)) {
            return 'LIPR-' . gmdate('mdHi') . '-' . (string) wp_rand(1000, 9999);
        }
    ...
    private function sanitize_po_segment(string $raw): string
    {
        ...
        // 4 + 1 + 8 + 1 + 8 = 22 max total for "LIPR-{first}-{last}".
        if (strlen($raw) > 8) {
            $raw = substr($raw, -8);
        }
