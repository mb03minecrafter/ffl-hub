...
final class RSRDealerBatchCronService extends AbstractOrderBatchCronService
{
    ...
    protected function send_risky_manual_hold_alert(
        ...
        $subject = (string) apply_filters(
            'fflhub_rsr_holosun_manual_risk_alert_subject',
            sprintf(
                '[FFLHub] RSR Holosun manual order UPC at risk: %s',
        ...
        $body = (string) apply_filters(
            'fflhub_rsr_holosun_manual_risk_alert_body',
        ...
        try {
            ...
            if (function_exists('error_log')) {
                error_log(self::LOG_PREFIX . ' Holosun manual risk alert failed: ' . $e->getMessage());
            }
        }
    }
    ...
    private function risk_alert_recipients(): array
    {
        ...
        $raw = apply_filters('fflhub_rsr_holosun_manual_risk_alert_recipients', $raw);
    ...
    private function risk_alert_body(array $rows, int $low_threshold, string $run_id): string
    {
        return '<div style="font-family:Arial,sans-serif;color:#1d2327;line-height:1.45;">'
            . '<h2 style="margin:0 0 12px;border-left:6px solid #d63638;padding-left:10px;">RSR Holosun Manual Order UPC At Risk</h2>'
            . '<p>An RSR Holosun dealer batch row was held for manual ordering instead of being priority-flushed automatically. One or more UPCs are at/under the low-stock risk rule and should be handled quickly in RSR.</p>'
            . $this->risk_alert_summary_table([
                'Run ID' => $run_id,
                'Low-stock threshold' => (string) $low_threshold,
                'Manual rows affected' => (string) count($rows),
                'Action needed' => 'Place or review the Holosun manual RSR order row.',
            ])
    ...
    private function risk_alert_rows_table(array $rows): string
    {
        $html = '<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:960px;">'
            ...
            . '<th align="right" style="border:1px solid #dcdcde;background:#f6f7f7;padding:8px;">RSR Stock</th>'
