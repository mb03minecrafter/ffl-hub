 ...
 * - Returns DTOs (OrderPlacementJobRow) and query results derived from table data.
 *
 * Rules:
...
final class OrderPlacementJobsRepository
{
     ...
     * @return string[] Job keys (lowercased; sorted).
     */
     ...
     * @param string $job_key Canonical key (dist|bucket).
     * @return OrderPlacementJobRow|null DTO if found, null if missing.
     */
    public static function get_job(int $order_id, string $job_key): ?OrderPlacementJobRow
    {
        global $wpdb;
     ...
     * @return OrderPlacementJobRow|null DTO if found, null if missing.
     */
    public static function get_job_for_order(WC_Order $order, string $job_key): ?OrderPlacementJobRow
    {
        return self::get_job((int) $order->get_id(), (string) $job_key);
    }

    /**
     * DTO-powered helper:
     * Return DistributorOrderLine[] from the job payload.
     *
     * @param int $order_id Woo order id.
     * @param string $job_key Canonical key (dist|bucket).
    ...
    {
        $job = self::get_job((int) $order_id, (string) $job_key);
     ...
     * Ordered by next_run_at ASC (nearest first), then id ASC for stability.
     *
     * IMPORTANT:
     ...
     * @return OrderPlacementJobRow[] List of DTOs.
     */
    public static function find_jobs_ready_for_processing(array $statuses, string $now_mysql_utc, int $limit): array
    {
        global $wpdb;
