    ...
    public static function refresh_changed_upcs(): array
    {
        ...
        $upserted = $wpdb->query("
            ...
            ON DUPLICATE KEY UPDATE
                has_changed = CASE
                    WHEN
                        NOT (product_id <=> VALUES(product_id))
                        OR NOT (upc <=> VALUES(upc))
                        OR NOT (distributor_id <=> VALUES(distributor_id))
                        OR NOT (distributor_product_id <=> VALUES(distributor_product_id))
                        OR NOT (distributor_sku <=> VALUES(distributor_sku))
                        OR NOT (manufacturer_norm <=> VALUES(manufacturer_norm))
                        OR NOT (qty <=> VALUES(qty))
                        OR NOT (stock_status <=> VALUES(stock_status))
                        OR NOT (dealer_price <=> VALUES(dealer_price))
                        OR NOT (shipping_cost <=> VALUES(shipping_cost))
                        OR NOT (landed_cost <=> VALUES(landed_cost))
                        OR NOT (map_price <=> VALUES(map_price))
                        OR NOT (msrp <=> VALUES(msrp))
                        OR NOT (ffl_required <=> VALUES(ffl_required))
                        OR NOT (sot_required <=> VALUES(sot_required))
                        OR NOT (dropship_enabled <=> VALUES(dropship_enabled))
                        OR NOT (enabled <=> VALUES(enabled))
                        OR NOT (shipping_weight_oz <=> VALUES(shipping_weight_oz))
                        OR NOT (shipping_length_in <=> VALUES(shipping_length_in))
                        OR NOT (shipping_width_in <=> VALUES(shipping_width_in))
                        OR NOT (shipping_height_in <=> VALUES(shipping_height_in))
                        OR NOT (source_updated_at <=> VALUES(source_updated_at))
                        OR NOT (source_offer_normalized_at <=> VALUES(source_offer_normalized_at))
                        OR NOT (selection_status <=> VALUES(selection_status))
                    THEN 1
                    ELSE has_changed
                END,
